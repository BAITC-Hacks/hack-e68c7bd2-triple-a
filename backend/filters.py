...
def summarize_no_match_reason(fail_reasons: dict[str, list[Reason]]) -> str:
    """Собирает человекочитаемое сообщение о том, почему никто не подошёл."""
    if not fail_reasons:
        return "Кандидаты есть, но ни один не подошёл по условиям запроса."

    counts: dict[str, int] = {}
    for reasons in fail_reasons.values():
        # считаем каждую причину один раз на кандидата (не дублируем, если у одного 2 причины)
    ...
    for reason, count in sorted(counts.items(), key=lambda kv: -kv[1]):
        label = REASON_LABELS_RU.get(reason, reason)
        parts.append(f"{count} из {total} — {label}")

    return "В этом городе и категории есть подрядчики, но никто не подошёл: " + "; ".join(parts) + "."
