...
describe('contractor search', () => {
  it('runs a demo after validation failure without leaving stale errors', async () => {
    const user = userEvent.setup();
    render(<App />);
    await submit(user);
    expect(screen.getByLabelText('Город')).toHaveAttribute('aria-invalid', 'true');
    await user.click(screen.getByRole('button', { name: /Свадьба в Алматы/ }));
    await screen.findByRole('heading', { name: 'Айдана Нур' });
    expect(screen.getByLabelText('Город')).toHaveAttribute('aria-invalid', 'false');
    expect(screen.getByLabelText('Дата события')).toHaveValue('2026-11-14');
    expect(searchMock.mock.calls[0]?.[0]).toMatchObject({ city: 'Алматы', category: 'Фотограф', budget_kzt: 300000 });
  });
  ...
  it('shows result details and treats text from the service as literal text', async () => {
    ...
    expect(within(card).getByText('Фотограф')).toBeInTheDocument();
    expect(within(card).getByText('Алматы')).toBeInTheDocument();
